"""Build JSONL banks of verified SC25 levels.

    python -m pebby.games.sc25.bank --levels N --seed S --difficulty D --split train --out path.jsonl
"""
...
def build(count, seed=0, difficulty=1, max_attempts=None, *, split):
    if count < 0:
        raise ValueError("count must be non-negative")
    max_attempts = max(1, 20 * count) if max_attempts is None else max_attempts
    specs = []
    tried = 0
...
def main(argv=None):
    ...
    specs, tried = build(args.levels, args.seed, args.difficulty, args.max_attempts,
                         split=args.split)
    save(specs, args.out)
