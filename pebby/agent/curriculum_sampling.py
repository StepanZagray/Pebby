...
def _weights(values, name):
    if isinstance(values, (str, bytes)):
        raise ValueError(f"{name} must contain five nonnegative weights")
    try:
        weights = torch.as_tensor(values, dtype=torch.float64, device="cpu")
    except (TypeError, ValueError, RuntimeError) as error:
        raise ValueError(f"{name} must contain five nonnegative weights") from error
    if weights.ndim != 1 or weights.numel() != len(DIFFICULTIES):
        raise ValueError(f"{name} must contain five weights")
...
class CurriculumSampler:
    ...
    def __init__(self, data, *, start=DEFAULT_START, end=DEFAULT_END):
        ...
        self._levels_by_difficulty = {difficulty: [] for difficulty in DIFFICULTIES}
        for seed in self.seeds.tolist():
            self._levels_by_difficulty[by_seed[seed]].append(seed)
        self._start = _weights(start, "start")
        self._end = _weights(end, "end")
        self.last_difficulty_counts = {difficulty: 0 for difficulty in DIFFICULTIES}
    ...
    def _quotas(ratios, batch_size):
        ...
        order = sorted(range(len(DIFFICULTIES)), key=lambda i: (-float(fractions[i]), i))
