...
class SpatialOutcomePlannerConfig:
    ...
    # Exact decoded HUD scalars (see hud_decoder) appended to the per-block
    # condition; 0 keeps the v1 parameter set and forward signature exactly.
    hud_scalars: int = 0
    # A trainable per-action score from the shared summary, added to the
    # outcome comparator's score. Lets the policy learn decisions the decoded
    # outcomes carry poorly (e.g. leaving an already-matching cycler). False
    # keeps the v1 parameter set exactly.
    direct_readout: bool = False

    def __post_init__(self):
        if not isinstance(self.direct_readout, bool):
            raise ValueError('direct_readout must be a bool')
        for name, value in asdict(self).items():
            if name == 'direct_readout':
                continue
            minimum = 0 if name == 'hud_scalars' else 1
            if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
                raise ValueError(f'{name} must be a positive integer' if minimum else f'{name} must be a nonnegative integer')
    ...
    def from_dict(cls, config):
        ...
        config.setdefault('hud_scalars', 0)
        config.setdefault('direct_readout', False)
        return cls(**config)
...
class SpatialOutcomePlanner(nn.Module):
    ...
    def __init__(self, config=None, **overrides):
        ...
        self.cfg = config
        width, condition = config.width, config.hud_width + 14 + config.hud_scalars + 4
        ...
        # Zero-initialised so a warm start reproduces the parent's scores exactly.
        self.direct_head = nn.Linear(config.summary, 1, bias=False) if config.direct_readout else None
        if self.direct_head is not None:
            nn.init.zeros_(self.direct_head.weight)

    def config(self):
        config = dict(architecture='spatial_outcome_planner', horizon=1, spatial_blocks=3,
                      dilations=[1, 2, 4], **asdict(self.cfg))
        if not self.cfg.hud_scalars:
            config.pop('hud_scalars')  # v1 checkpoints round-trip byte-for-byte.
        if not self.cfg.direct_readout:
            config.pop('direct_readout')
        return config
    ...
    def _hud_scalars(self, raw, hud_scalars):
        expected = self.cfg.hud_scalars
        if not expected:
            if hud_scalars is not None:
                raise ValueError('this planner was configured without hud_scalars')
            return None
        if (not torch.is_tensor(hud_scalars) or hud_scalars.shape != (raw.size(0), expected)
                or hud_scalars.device != raw.device or not hud_scalars.is_floating_point()):
            raise ValueError(f'hud_scalars must be a floating tensor of shape ({raw.size(0)}, {expected}) on the input device')
        if not bool(torch.isfinite(hud_scalars).all()):
            raise ValueError('hud_scalars must be finite')
        return hud_scalars

    def forward(self, raw, state, glyph, player_probabilities, *, action_rows=None, hud_scalars=None):
        actions = self._inputs(raw, state, glyph, player_probabilities, action_rows)
        scalars = self._hud_scalars(raw, hud_scalars)
        ...
        shared = torch.cat((hud, glyph.to(hud.dtype)), -1)
        if scalars is not None:  # Layout: [hud_width | 14 glyph | hud_scalars | 4 action].
            shared = torch.cat((shared, scalars.to(hud.dtype)), -1)
        shared = shared[:, None].expand(-1, 4, -1)
        ...
        action_logits = self.score_outcomes(fields, value, events)
        if self.direct_head is not None:
            action_logits = action_logits + self.direct_head(summary).squeeze(-1).float()
        return dict(action_logits=action_logits, field_logits=fields,
