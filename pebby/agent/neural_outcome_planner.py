...
def neural_outcome_losses(predictions, batch, weights=None, *, skip_losses=()):
    ...
    ``skip_losses`` omits named loss computations while retaining every target
    validation and diagnostic. Wrappers can supply balanced replacements.
    DEFAULT_WEIGHTS governs this base objective; wrappers own their final sum.
    """
    supplied = {} if weights is None else dict(weights)
    if set(supplied) - DEFAULT_WEIGHTS.keys():
        raise ValueError('unknown loss family')
    if (not isinstance(skip_losses, (tuple, list, set, frozenset))
            or any(name not in DEFAULT_WEIGHTS for name in skip_losses)):
        raise ValueError('unknown skipped loss family')
    ...
    losses = {}
    if 'physical' not in skip_losses:
        losses['physical'] = torch.stack([F.cross_entropy(scores.float().flatten(0, 1), target.flatten())
                                         for scores, target in zip(fields, targets)]).mean()
    if 'value' not in skip_losses:
        losses['value'] = F.cross_entropy(value.float().flatten(0, 1), bins.flatten())
    if 'events' not in skip_losses:
        losses['events'] = F.binary_cross_entropy_with_logits(events.float(), event_targets)
    if 'policy' not in skip_losses:
        losses['policy'] = -(policy_targets * policy_log_probabilities).sum() / valid_count.clamp_min(1)
    with torch.no_grad():
    ...
    total = sum(weights[name] * loss for name, loss in losses.items()) if losses else logits.sum() * 0
    return dict(total=total, losses=losses,
