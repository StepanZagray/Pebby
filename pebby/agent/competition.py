...
class DiverseLivesDecision:
    """Four-logit policy through `DiverseLivesController`; RESET only on GAME_OVER.

    Same interface as `FourMovementDecision`. The controller's stall mask,
    reversal/repeat penalties and per-life seeded sampling are runtime
    heuristics, not learning: voluntary RESET is still not learned and there is
    still no persistent game memory. Every knob is recorded in `metadata`.
    """

    def __init__(self, policy, device="cpu", **knobs):
        from .diverse_controller import DiverseLivesController
        self.controller = DiverseLivesController(policy, device, **knobs)
        self.metadata = dict(reset_controller="game_over_only", voluntary_reset_learned=False,
                             policy_action_ids=[1, 2, 3, 4], decision_action_ids=[0, 1, 2, 3, 4],
                             action_selection="diverse_lives",
                             stall_controller="unchanged_frame_mask" if self.controller.stall_mask
                                              else "none",
                             history_boundary_semantics="clear_on_life_loss_level_transition_and_reset",
                             persistent_game_memory=False, learned=False, runtime_heuristics=True,
                             controller=dict(self.controller.metadata))

    def start(self, context):
        self.controller.start(context.frame, level_index=context.levels_completed)

    def decide(self, context):
        if context.state == GameState.GAME_OVER:
            return 0
        return self.controller.decide(context.frame, level_index=context.levels_completed,
                                      state=context.state) + 1

    def observe(self, observation, event):
        if observation.frame is None:
            return
        self.controller.observe(observation.frame, None if event["action"] == 0 else event["action"] - 1,
                                life_lost=event["lives_after"] < event["lives_before"],
                                level_changed=event["level_after"] != event["level_before"],
                                reset=event["reset"])
