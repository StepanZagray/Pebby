...
class RecoveryComparisonTests(unittest.TestCase):
    ...
    def test_fresh_arms_discard_updates_and_checkpoint_preserves_contract(self):
        ...
        args = SimpleNamespace(steps=780, lr=.0001, cache=Path('base'), supplement=Path('extra'),
                               recovery_fraction=.25, policy_row_fraction=.75, distance_ordering=.2)
        ...
        self.assertEqual(output['distance_ordering'], .2)
        self.assertEqual(output['qualification_sha256'], 'qualification')
    ...
    def test_quality_manifest_binds_indices_and_refuses_old_qualification(self):
        ...
        report.update(objective_weights={'unchanged': True}, initial_planner_weights_sha256='head',
                      ...
                      recovery_fraction=.25, policy_row_fraction=.75, distance_ordering=0.,
                      requested_arms=['recovery'], offline_diagnostics=False, gameplay_baseline_sha256='baseline')
        qualification = dict(report, status='complete', qualification_passed=True)
        trainer.validate_qualification(qualification, report)
        for key in ('quality_manifest_sha256', 'quality_row_file_sha256', 'quality_filter', 'distance_ordering', 'requested_arms', 'offline_diagnostics', 'gameplay_baseline_sha256'):
        ...
        with self.assertRaisesRegex(ValueError, 'quality-gated'):
            trainer.validate_qualification({**qualification, 'distance_ordering': .2}, report)
