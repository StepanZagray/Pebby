"""Closing-K4 production tests use only constructed generated-engine corridors."""
...
from pebby.agent import world_data as wd,world_model,world_rollout as helper
...
class ClosingRolloutTests(unittest.TestCase):
    @classmethod
    ...
    def loss(self,model,batch):return objectives.world_losses(model,batch,{'successor_policy':1.},sigreg_generator=torch.Generator().manual_seed(8))
    ...
    def test_actual_generated_final_win_and_reset_target_histories_and_labels(self):
        for kind in ('win','reset','deadend'):
            with self.subTest(kind=kind):
                ...
                expected=objectives._value_loss(model,out['predicted'].flatten(0,1),batch['distances'].long().flatten(),batch['terminal'].flatten(),batch['won'].flatten(),batch['lost_life'].flatten())[0]
    ...
    def test_forbid_interior_closing_flags_and_keep_legacy_parity(self):
        ...
        # Two genuine ordinary rows: absent rollout fields must match an
        # explicit all-false mask, without comparing a function to itself.
        plain={k:v[1:].repeat((2,)+(1,)*(v.ndim-1)) for k,v in batch.items()
               if k not in ('rollout_mask','rollout_actions')}
        explicit={**plain,'rollout_mask':torch.zeros(2,dtype=torch.bool),
                  'rollout_actions':torch.full((2,4),-1,dtype=torch.long)}
        a=self.loss(model,plain);b=self.loss(model,explicit)
        for name in ('total','logits','targets','predicted'):
