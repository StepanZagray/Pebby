        ...
        with tempfile.TemporaryDirectory() as directory:
            for index, envelope in enumerate(envelopes):
                with self.subTest(format=envelope['format']):
                    ...
                    import inference
                    # The argmax controller preserves the plain H8 scores; the
                    # diverse controller is covered by tests/test_inference_controller.
                    engine = inference.Engine(path, banks=object(), controller='argmax')
                    engine.agent = agent
                    def fake_replay(level, actions, history=None, observer=None):
                        self.assertIsInstance(observer, inference.HistoryObserver)
                        observer.start(frames[0].tolist())
                        self.assertIsInstance(observer.history, PolicyHistory)
                        for i, frame in enumerate(frames[1:], 1):
                            observer.observe(frame.tolist(), i % 4, life_lost=False, level_changed=False, reset=False)
                        return object(), [frames[-1]], frames[-1]
                    with patch.object(inference, 'replay', side_effect=fake_replay), \
                         patch.object(inference, 'status_of', return_value={'won': False, 'finished': False}):
                        response = engine.act({}, [])
