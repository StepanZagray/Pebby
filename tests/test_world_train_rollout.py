"""Production rollout trainer regressions using generated engine trajectories."""
...
from pebby.agent import world_train as trainer, world_model
...
class TrainerRolloutTests(unittest.TestCase):
    @classmethod
    ...
    def test_real_sequence_sampler_path_reaches_loss_and_retains_current_inputs(self):
        ...
        def loss(model,batch,*args,**kwargs):
            ...
            return objectives.world_losses(model,batch,*args,**kwargs)
    ...
    def test_cli_persists_source_bound_index_in_all_checkpoints_and_report(self):
        ...
        args=['--train',str(self.fixture.source),'--on-policy-data',str(self.fixture.source),
              ...
              '--checkpoint-out',str(out),'--seed','5','--select-on','last']
        ...
        counts = report['history'][0]['train']
        self.assertEqual(counts['rollout_rows'], counts['samples'] // 2)
        self.assertEqual(counts['counterfactual_rows'], counts['samples'] // 2)
