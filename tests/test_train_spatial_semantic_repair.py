...
class ProspectiveSelectionTests(unittest.TestCase):
    ...
    def test_offline_selection_rejected_and_final_snapshot_does_not_alias(self):
        with self.assertRaisesRegex(ValueError, 'offline checkpoint selection'):
            trainer.CheckpointSelection('validation-policy')
        with mock.patch.object(trainer, 'guard') as guard:
            with self.assertRaises(SystemExit):
                trainer.main(['--out', 'unused', '--selection', 'validation-policy'])
            guard.assert_not_called()
        model = torch.nn.Linear(1, 1, bias=False)
        selector = trainer.CheckpointSelection('final')
        provenance = dict(sampling_sha256='prefix', tier_counts={'1': 10})
        for step, score in enumerate([.5, .2, .8], 1):
        ...
        self.assertEqual(float(selector.selected['weights']['weight']), 3.)
        self.assertEqual(selector.selected['provenance']['tier_counts']['1'], 10)
        meta = trainer.selection_metadata(SimpleNamespace(steps=3, selection='final', validation_every=1), selector.selected, 'selected')
        self.assertEqual(meta['criterion_value'], 3)
        self.assertFalse(meta['offline_metrics_used_for_selection'])
        self.assertFalse(meta['promoted'])
        self.assertEqual(meta['gameplay_status'], 'gameplay_not_evaluated')
    ...
    def test_final_mode_never_chooses_earlier_better_loss_and_none_is_explicit(self):
        ...
        empty = trainer.CheckpointSelection('final')
        empty.observe(model, 2, 2, {'tier_balanced_policy_set_nll': None}, {})
        self.assertEqual(empty.selected['step'], 2); self.assertEqual(empty.final['step'], 2)
    ...
    def test_checkpoint_actual_steps_and_sampling_prefix_match_saved_weights(self):
        ...
        args = SimpleNamespace(steps=3, selection='final', validation_every=2, lr=.001, tier_counts=[64, 0, 0, 0, 0, 0, 0],
