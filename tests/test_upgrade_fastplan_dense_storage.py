...
from tools import upgrade_fastplan_dense_storage as m

PY_OLD='''ABI_VERSION = 2
class Tables:
    safe = 7
class Params:
    safe = 9
...
'''
PY_NEW=PY_OLD
C_OLD=(Path(__file__).parent/'fixtures/fastplan_before_dense_storage.c').read_text()
C_NEW=m.compact_graph_storage(C_OLD)

class DenseStorageMigrationTests(unittest.TestCase):
    def fixture(self,root):
        ...
        validation_job=r.jobs_for(1,'validation',set(),seed_range=cfg['validation_seed_range'])[0]
        retry=dict(status='rejected',next_seed=validation_job['seed']+1,
                   failures=[],error='generated fixture retry')
        r._checkpoint(bank/'jobs/validation-000000.json',validation_job,r._hash(manifest),retry)
        receipt=root/'validation.json';r._write(receipt,dict(format=m.RECEIPT_FORMAT,status='complete',
            ...
            bfs_order_unchanged=True,graph_and_distances_unchanged=True,equality_verified=True))
    ...
    def test_rejects_solver_python_and_c_edits(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            with self.assertRaisesRegex(ValueError,'byte-identical'):m.verify_python(old,new)
            old.write_text(C_OLD)
            for text in (C_NEW.replace('return 2;','return 3;'),C_NEW+'\nint surprise(void) {return 0;}\n',
