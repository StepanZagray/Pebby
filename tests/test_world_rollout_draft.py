"""CPU regressions for draft mixed K4 losses; real generated engine trajectories only."""
import copy
...
from pebby.agent import world_model as legacy, world_data, world_grounding
...
path = Path(__file__).resolve().parents[1] / 'artifacts/world_model_multistep_draft.py'
spec = importlib.util.spec_from_file_location('pebby.agent._multistep_draft', path)
draft = importlib.util.module_from_spec(spec)
spec.loader.exec_module(draft)


class MixedRolloutDraftTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def loss(self, model, batch=None, **kwargs):
        return draft.world_losses(model,self.batch if batch is None else batch,
    ...
    def test_histories_match_independent_real_engine_encodes_and_cost(self):
        ...
        base=legacy.world_losses(model,ordinary,{'successor_policy':1.},sigreg_generator=torch.Generator().manual_seed(7))
    ...
    def test_chronological_grounding_value_glyph_and_successor_policy_alignment(self):
        ...
        expected=draft._value_loss(model,out['predicted'].flatten(0,1),b['distances'].long().flatten(),
                                  b['terminal'].flatten(),b['won'].flatten())[0]
        torch.testing.assert_close(out['losses']['imagined_value'],expected)
        masks=b['next_optimal'].flatten();valid=masks!=0;bits=draft.optimal_bits(masks[valid])
    ...
    def test_counterfactual_metrics_exclude_sequence_and_horizons_are_explicit(self):
        ...
        rank,top=draft._rank_of_true(out['predicted'][1:],out['targets'][1:])
    ...
    def test_legacy_absent_and_false_masks_keep_outputs_losses_and_gradients(self):
        model=self.model();b={key:value for key,value in self.batch.items() if key not in ('rollout_mask','rollout_actions')}
        a=legacy.world_losses(model,b,{'successor_policy':1.},sigreg_generator=torch.Generator().manual_seed(7))
        result=self.loss(model,b)
        for key in ('logits','latent','targets','predicted','total'):torch.testing.assert_close(result[key],a[key],atol=0,rtol=0)
        for key in a['losses']:torch.testing.assert_close(result['losses'][key],a['losses'][key],atol=0,rtol=0)
        # Archived drafts predate the new failure/coverage diagnostics. Compare every legacy metric.
        self.assertTrue(set(result['diagnostics']) <= set(a['diagnostics']))
        for key in result['diagnostics']:torch.testing.assert_close(result['diagnostics'][key],a['diagnostics'][key],atol=0,rtol=0)
        a['total'].backward()
        gradients={key:p.grad.clone() if p.grad is not None else None for key,p in model.named_parameters()}
        model.zero_grad(set_to_none=True);result['total'].backward()
        for key,p in model.named_parameters():
            if gradients[key] is None:self.assertIsNone(p.grad)
            else:torch.testing.assert_close(p.grad,gradients[key],atol=0,rtol=0)
        false={**b,'rollout_mask':torch.zeros(2,dtype=torch.bool),'rollout_actions':torch.full((2,4),-1)}
        c=self.loss(model,false);torch.testing.assert_close(c['total'],a['total'],atol=0,rtol=0)
