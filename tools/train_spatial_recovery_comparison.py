...
require audited nonzero-policy-target row allowlists, retain the original TRAIN loss weights, and evaluate actual sequential gameplay. Cached validation is opt-in diagnostic evidence.
Qualification updates are disposable. The optional distance-ordering objective is
explicitly bound to qualification; its default preserves the original objective.
"""
import argparse
import copy
...
def validate_qualification(qualification, report):
    keys = ('source_sha256', 'cache_manifest_sha256', 'supplemental_manifest_sha256', 'objective_weights',
            ...
            'quality_row_file_sha256', 'quality_filter', 'distance_ordering', 'requested_arms',
            'offline_diagnostics', 'gameplay_baseline_sha256')
...
def checkpoint_for(checkpoint, model, arm, args, report):
    ...
    result.update(planner_weights={k: v.detach().cpu().clone() for k, v in model.state_dict().items()},
                  ...
                  policy_row_fraction=args.policy_row_fraction, distance_ordering=args.distance_ordering,
                  qualification_sha256=report['qualification_sha256'],
                  promoted=False, candidate_only=True, offline_metrics_select_checkpoint=False,
                  gameplay=report.get('arms', {}).get(arm, {}).get('gameplay'),
                  gameplay_gate=report.get('arms', {}).get(arm, {}).get('gameplay_gate'),
...
def run_arm(args, checkpoint, arrays, supplement, sampler, player_weights, weights, report, arm):
    ...
    for step in range(steps):
        ...
        record, norm = fit_step(model, optimizer, items, player_weights, weights, args.precision, distance_ordering=args.distance_ordering)
        if (step + 1) % 25 == 0 or step + 1 == steps:
            torch.cuda.synchronize()
            update = dict(step=step + 1, loss=float(record['total'].detach()), metrics=scalar_metrics(record),
    ...
    if not args.qualify:
        ...
        from pebby.agent.spatial_outcome_policy import SpatialOutcomePolicy
        ...
        policy = SpatialOutcomePolicy(encoder, model).to('cuda').eval()
        entry['gameplay'] = evaluate_sequential(policy, 'cuda', guard, per_level_cap=300)
        entry['gameplay_gate'] = assess_gameplay(report['gameplay_baseline'], entry['gameplay'])
        print(json.dumps(dict(event='gameplay_result', arm=arm,
                              levels_completed=entry['gameplay']['levels_completed'],
                              gate=entry['gameplay_gate']['status'])), flush=True)
        write(args.out_dir / 'report.json', report)
        del policy, encoder
        if args.offline_diagnostics:
            entry['offline_diagnostics'] = evaluate(model, arrays['validation'], player_weights, weights, BATCH_SIZE, distance_ordering=args.distance_ordering)
...
def main(argv=None):
    ...
    parser.add_argument('--gameplay-baseline', type=Path, default=ROOT/'artifacts/spatial-recovery-v1/quality-fit/recovery.pt')
    ...
    if sha(args.gameplay_baseline) != args.gameplay_baseline_sha256:
        parser.error('gameplay baseline checksum differs')
    ...
    report = dict(status='running', pid=os.getpid(), started_local=datetime.now().astimezone().isoformat(),
                  ...
                  policy_row_fraction=args.policy_row_fraction, distance_ordering=args.distance_ordering, arms={},
                  requested_arms=args.arms, offline_diagnostics=args.offline_diagnostics,
                  gameplay_baseline_sha256=args.gameplay_baseline_sha256,
                  evaluation_priority='actual_sequential_gameplay', promoted=False,
    ...
    try:
        ...
        bindings.add(args.gameplay_baseline, args.gameplay_baseline_sha256)
        ...
        if not args.qualify:
            ...
            from pebby.agent.gameplay_gate import evaluate_sequential
            from pebby.agent.spatial_outcome_policy import load_checkpoint
            baseline_policy, _ = load_checkpoint(args.gameplay_baseline, 'cuda')
            report['gameplay_baseline'] = evaluate_sequential(baseline_policy, 'cuda', guard, per_level_cap=300)
            del baseline_policy
            write(args.out_dir / 'report.json', report)
        ...
        if len(args.arms) == 2 and report['arms']['control']['first_batch'] != report['arms']['recovery']['first_batch']:
