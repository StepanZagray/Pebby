...
def rollout(policy, env, *, max_actions=300, device='cpu', optimal=None, check=lambda: None, on_stall='repeat'):
    if on_stall not in ('repeat', 'next-best'):
        raise ValueError('on_stall must be repeat or next-best')
    ...
    try:
        run = evaluate.rollout(policy, ObservedEnvironment(env, diagnostics, check), max_actions,
                               torch.device(device), optimal, on_stall=on_stall, temperature=0.)
...
def main(argv=None):
    ...
    parser.add_argument('--on-stall', choices=('repeat', 'next-best'), default='repeat',
                        help='repeat preserves strict neural selection; next-best uses refusal fallback')
    ...
    report = dict(status='running', pid=os.getpid(), started_local=datetime.now().astimezone().isoformat(),
        ...
        on_stall=args.on_stall, temperature=0., native_lives=3, planner_horizon=1, planner_refinement_loops=1,
        ...
        for name, path, expected in args.checkpoint:
            ...
            for index, (level, optimum, spec) in enumerate(zip(levels, optima, selected)):
                ...
                run = rollout(policy, Ls20Scenario(level, int(spec.get('training_context_index',0))),
                              device=args.device, optimal=optimum, check=check, on_stall=args.on_stall)
