...
from pebby.agent.world_sequences import build_sidecar,load_sidecar,MODES
...
def main(argv=None):
    ...
    p.add_argument('--mode',choices=MODES,default='on_policy_train')
    ...
    index=build_sidecar(data,args.source,args.out,args.mode)
    checked=load_sidecar(args.out,args.source,data,mode=args.mode)
    ...
    report={**index.meta,'status':'complete','pid':os.getpid(),'device':'cpu','torch_threads':1,
            ...
            'eligible_levels_by_difficulty':{str(d):len({int(data['seeds'][i]) for i in checked.anchor_row if by_seed[int(data['seeds'][i])]==d}) for d in range(1,6)},
