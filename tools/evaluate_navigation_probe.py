...
def load_policy(path, device):
    from pebby.agent import navigation_probe, spatial_outcome_policy, neural_imagination_policy, repaired_imagination_policy, joint_goal_planning
    envelope = torch.load(path, map_location='cpu', weights_only=True)
    loaders = {joint_goal_planning.FORMAT: joint_goal_planning.load_checkpoint,
               repaired_imagination_policy.FORMAT: repaired_imagination_policy.load_checkpoint,
               spatial_outcome_policy.FORMAT: spatial_outcome_policy.load_checkpoint,
               navigation_probe.FORMAT: navigation_probe.load_checkpoint,
               neural_imagination_policy.FORMAT: neural_imagination_policy.load_checkpoint}
    if envelope.get('format') not in loaders:
        raise ValueError('unsupported experimental checkpoint format')
    load = loaders[envelope['format']]
...
def main(argv=None):
    ...
    report = dict(format='pebby.navigation-evaluation.v1', status='running', pid=os.getpid(),
                  ...
                  limits=['Local diagnostic, not an official API scorecard.',
                          ...
                          'No learned voluntary RESET or persistent game memory; imagination checkpoints use neural trajectory selection.'])
