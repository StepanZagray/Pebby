...
def evaluate(model, arrays, player_weights, objective_weights, size):
    ...
    with torch.inference_mode():
        for start in range(0, len(arrays['seeds']), size):
            ...
            record = forward(model, batch(arrays, np.arange(start, min(start+size, len(arrays['seeds']))), 'cuda'),
                             player_weights, objective_weights, 'float32')
...
def qualify(args, arrays, sampler, player_weights, objective_weights, report):
    ...
    for size in (1024, 512, 256, 128, 64):
        ...
        try:
            ...
            record, norm = fit_step(model, optimizer, items, player_weights, objective_weights, args.precision)
    ...
    for step in range(args.tiny_steps):
        guard()
        fit_step(model, optimizer, items, player_weights, objective_weights, args.precision)
        with torch.no_grad():
            record = forward(model, items, player_weights, objective_weights, args.precision)
    ...
    model.eval()
    with torch.no_grad():
        final = forward(model, items, player_weights, objective_weights, 'float32')
...
def train(args, arrays, sampler, player_weights, objective_weights, parent, report):
    ...
    for step in range(args.steps):
        ...
        model.train()
        record,norm = fit_step(model, optimizer, batch(arrays['train'],rows,'cuda'), player_weights,objective_weights,args.precision)
    ...
    report['validation'] = evaluate(model, arrays['validation'], player_weights, objective_weights, args.batch_size)
    checkpoint = dict(format=FORMAT, encoder_config=parent['config'], encoder_weights=parent['weights'],
        ...
        batch_size=args.batch_size, learning_rate=args.lr, weight_decay=.05, objective_weights=objective_weights,
...
def main(argv=None):
    ...
    report = dict(status='running',pid=os.getpid(),start_ticks=Path(f'/proc/{os.getpid()}/stat').read_text().split()[21],
                  ...
                  official_inputs_used=False, encoder_frozen=True, learning_rate=args.lr, precision=args.precision,
    ...
    sources += [ROOT/'pebby/agent'/name for name in ('spatial_outcome_planner.py','spatial_outcome_policy.py','spatial_outcome_objective.py',
                'neural_outcome_planner.py','neural_outcome_policy.py','world_model.py','world_train.py','world_grounding.py','curriculum_sampling.py')]
