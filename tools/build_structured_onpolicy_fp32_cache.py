...
to FP32 and stored directly in FP32, matching legacy imagined policy caches.
Current and actual-successor fields remain FP16. The v1 builder is untouched.
"""
import argparse
import json
import os
...
FORMAT = 'pebby.structured-onpolicy-field-cache.v2'
...
def build(data,out,policy,hashes,*,device='cpu',batch_size=32,max_encoder_batch=128):
    ...
    try:
        ...
        imagined=np.lib.format.open_memmap(temporary/'imagined_fields.npy',mode='w+',dtype=np.float32,shape=(n,4,148,96))
        actual_error=0.
        with torch.inference_mode(),torch.autocast(device_type=torch.device(device).type,enabled=False):
            for first in range(0,n,batch_size):
                ...
                if not np.isfinite(predicted).all():raise ValueError('nonfinite imagined FP32 fields')
                fields[first:end]=current;next_fields[first:end]=future;imagined[first:end]=predicted
        ...
        manifest={'format':FORMAT,'status':'complete','source':'generated_only','split':'train',
            ...
            'precision':{'compute_dtype':'float32','current_storage_dtype':'float16',
                'actual_storage_dtype':'float16','imagined_storage_dtype':'float32','autocast':False,
                'matmul_tf32':False,'cudnn_tf32':False,'actual_quantization_max_error':actual_error,
                'imagined_quantization':False,'imagined_input':'stored current FP16 promoted to FP32'},
