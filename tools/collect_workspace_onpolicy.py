"""Collect a bounded generated TRAIN on-policy pilot for a workspace checkpoint.
...
The default pilot is 32 stratified levels from the original TRAIN bank and 32
from the extended TRAIN bank.  It is preparation for a later cache/training
...
DEFAULT_OUT = Path("data/ls20-world-workspace-onpolicy-pilot-train.npz")
DEFAULT_REPORT = Path("artifacts/world-workspace-onpolicy-pilot.json")
MAX_ACTIONS = 48
HISTORY = 8
...
def select_sources(old_bank: Path, extended_bank: Path, combined_train: Path,
                   ...
                   array_cache: Path = DEFAULT_ARRAY_CACHE, per_source: int = 32,
                   selection_seed: int = 20260912):
    """Select deterministic equal-difficulty source subsets from TRAIN only."""
    if type(per_source) is not int or not 1 <= per_source <= 32:
        raise ValueError("per_source must be in 1..32 for the bounded pilot")
    ...
    if len(selected) != 2 * per_source or len(set(selected)) != len(selected):
        raise ValueError("source selection did not produce distinct 32/source TRAIN levels")
...
def run_collection(*, checkpoint=CHECKPOINT, old_bank=DEFAULT_OLD_BANK,
                   ...
                   out=DEFAULT_OUT, report=DEFAULT_REPORT, per_source=32, workers=1,
                   selection_seed=20260912, deadline_seconds=900):
    """Run the bounded pilot; kept separate from CLI so tests never launch it."""
    ...
    report_data = {
        ...
        "selection_contract": "32 deterministic equal-difficulty levels from each disjoint TRAIN bank, "
                              "intersected with combined TRAIN seeds and eligible cached seeds.",
    }
