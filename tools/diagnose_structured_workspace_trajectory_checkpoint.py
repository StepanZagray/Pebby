...
def semantic_before(env, teacher, choice, spec):
    """Diagnostic engine facts, read only after the public action is committed."""
    state=env.snapshot();dx,dy=names.ACTION_DELTAS[choice]
    target=(state['cell'][0]+dx,state['cell'][1]+dy)
    result={'state':state,'attempted_neighbor':target,'goal':None,'wall_or_edge':None,
            'cycler_at_next_tick':None,'moving_cycler_at_next_tick':None,'tick':None}
    if spec is not None:
        result['wall_or_edge']=target in map(tuple,spec['walls']) or not (0<=target[0]<12 and 0<=target[1]<12)
        for index,goal in enumerate(spec['goals']):
            if tuple(goal['cell'])==target:
                solved=bool(state['goals_solved'][index])
                result['goal']={'index':index,'triple':goal['triple'],'solved':solved,
                    'mismatched_fields':[name for name,a,b in zip(('shape','color','rotation'),state['triple'],goal['triple']) if a!=b],
                    'unsolved_mismatch':not solved and tuple(goal['triple'])!=tuple(state['triple'])}
    oracle=getattr(teacher,'oracle',None)
    if oracle is not None:
        logical=oracle.state_of(env);tick=logical[-1];following=oracle.layout.next_tick(tick)
        result.update(tick=int(tick),next_tick=int(following),
            cycler_at_next_tick=oracle.layout.cycler_at(target,following),
            moving_cycler_at_next_tick=oracle.layout.moving_cyclers[following].get(target),
            refill_available=target in oracle.refills and not bool(logical[5]&(1<<oracle.refills.index(target))),
            launcher_triggers=[{'delta':l['delta'],'distance':l['distance']} for l in oracle.layout.launchers if target in l['triggers']])
    return result


def semantic_after(before, after, evidence, observation):
    old=before['state'];lost=after['lives']<old['lives'];outcome=evidence[2] if evidence else None
    goal=before['goal']
    return {'logical_outcome':outcome,'mismatched_goal_attempt':bool(goal and goal['unsolved_mismatch']),
        'wall_or_edge_attempt':before['wall_or_edge'] is True,
        'rejected_goal':outcome=='rejected','position_unchanged':tuple(old['cell'])==tuple(after['cell']),
        'static_cycler_attempt':before['cycler_at_next_tick'] is not None and before['moving_cycler_at_next_tick'] is None,
        'moving_cycler_attempt':before['moving_cycler_at_next_tick'] is not None,
        'launcher_fired':outcome=='launched',
        'shape_changed_without_reset':not lost and old['triple'][0]!=after['triple'][0],
        'color_changed_without_reset':not lost and old['triple'][1]!=after['triple'][1],
        'rotation_changed_without_reset':not lost and old['triple'][2]!=after['triple'][2],
        'budget_increased_without_reset':not lost and after['steps_left']>old['steps_left'],
        'life_loss_budget_exhaustion_verified':lost and outcome=='died' and evidence[1][6]<0,
        'budget_delta_observed':after['steps_left']-old['steps_left'],
        'goals_cleared_without_reset':0 if lost else sum(after['goals_solved'])-sum(old['goals_solved'])}
...
def diagnose_level(policy, env, teacher, max_actions=200, device='cpu', progress=None, spec=None):
    ...
    decisions = []; ending = 'capped'; consecutive = 0; life_index = 0; first_by_life = {}; visits = Counter()
    for step in range(max_actions):
        ...
        before_facts = semantic_before(env, teacher, choice, spec)
        repeat_key = json.dumps(before_facts['state'], sort_keys=True) + str(before_facts.get('tick'))
        prior_visits = visits[repeat_key]; visits[repeat_key] += 1
        ...
        row = {'step': step, 'life_index': life_index, 'action_index': choice, **label,
               ...
               'logits': logits.detach().cpu().tolist(), 'choice_probability': float(logits.float().softmax(0)[choice]),
               'prior_identical_logical_state_visits': prior_visits, 'before': before_facts}
        ...
        row['after'] = env.snapshot()
        row['semantic'] = semantic_after(before_facts, row['after'], evidence, observation)
...
def main():
    ...
    try:
        ...
        if sources[str(args.checkpoint)] != args.checkpoint_sha256: raise ValueError('explicit checkpoint SHA mismatch')
        ...
        if provenance.get('smoke') is not False or type(provenance.get('updates')) is not int or provenance.get('updates',0)<1 or provenance.get('batch_size')!=1024:
            raise ValueError('production fixed-schedule B1024 workspace checkpoint required')
        ...
        for level,spec in zip(levels[:args.levels],specs[:args.levels]):
            ...
            def progress(rows):
                ...
                if len(rows)%100==0:persist()
            entry.update(diagnose_level(policy,env,teacher,device=args.device,progress=progress,spec=spec))
        ...
        report['gameplay_summary'] = dict(Counter(e['summary']['ending'] for e in report['levels']))
        report['semantic_counts'] = dict(Counter(k for e in report['levels'] for r in e['decisions'] for k,v in r['semantic'].items() if v is True))
