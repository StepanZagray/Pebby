...
def selection_mode(value):
    if value != 'final':
        raise argparse.ArgumentTypeError('offline checkpoint selection is disabled; use final to save an unpromoted candidate, then evaluate actual gameplay')
    return value


class CheckpointSelection:
    """Independent final CPU snapshots; offline scores are diagnostics only."""
    def __init__(self, mode):
        if mode != 'final':
            raise ValueError('offline checkpoint selection is disabled; only final candidates are supported')
...
def selection_metadata(args, snapshot, role):
    ...
    return dict(role=role, mode=args.selection, selected_step=snapshot['step'],
        ...
        criterion='fixed final optimizer step',
        validation_criterion_value=snapshot['criterion_value'],
        criterion_value=snapshot['step'], validation_every=args.validation_every,
        validation_steps=list(validation_steps(args.steps, args.validation_every)),
        offline_metrics_used_for_selection=False, candidate_only=True, promoted=False,
        gameplay_status='gameplay_not_evaluated',
...
def main(argv=None):
    ...
    parser.add_argument('--selection', type=selection_mode, default='final')
    ...
    report = dict(status='running', pid=os.getpid(), started_local=datetime.now().astimezone().isoformat(),
        ...
        selection_protocol=dict(mode=args.selection, validation_every=args.validation_every,
            ...
            offline_metrics_used_for_selection=False, candidate_only=True, promoted=False,
            gameplay_status='gameplay_not_evaluated', unsupported_tiers='excluded and reported; all unsupported gives None',
